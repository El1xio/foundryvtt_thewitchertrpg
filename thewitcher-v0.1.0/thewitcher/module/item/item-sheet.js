const SIMPLE_ITEMS = ["service", "general_gear", "food_and_drink"];
const EXTENDED_ITEMS = ["container", "alchemical_item"];
export default class WitcherItemSheet extends ItemSheet {
    ...
    getData(options) {
        ...
        // @ts-ignore
        data.isContainer = data.item.data.hasOwnProperty("container");
        // @ts-ignore
        data.hasEffect = data.item.data.hasOwnProperty("effect");
        return data;
    }
    static getItemStatus(item) {
        if (["service", "general_gear", "food_and_drink"].includes(item.type))
            return;
    }
